...
class Tindakanberobat extends CI_Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Tbl_pendaftaran_rekmed_model');
   ...
   public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'index.php/tindakanberobat/index?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'index.php/tindakanberobat/index?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'index.php/tindakanberobat/index';
            $config['first_url'] = base_url() . 'index.php/tindakanberobat/index';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tbl_pendaftaran_rekmed_model->total_rows($q);
        $pendaftaran = $this->Tbl_pendaftaran_rekmed_model->get_limit_data($config['per_page'], $start, $q);
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        $pdf->Cell(49, 7, $r->tanggal_daftar, 1, 0, 'C');
  ...
  function cetakantrian($id){
        ...
        $sql_pasien = $this->Tbl_pendaftaran_rekmed_model->get_by_id($id);
        $this->load->library('pdf');
        $pdf = new FPDF();
        $pdf->AddPage('P','A4');
        ...
        $pdf->Cell(90,5,'PUSKESMAS NUSAMANDIRI',0,0,'C');
        $pdf->Cell(10,5,'',0,0,'C');
        ...
        $pdf->Cell(10,5,'',0,0,'C');
        $pdf->setFont('Arial','',7);
        $pdf->Cell(90,5,'- Kartu ini hanya bisa digunakan untuk berobat di puskesmas nusamandiri.',0,1,'L');
    ...
    public function read() 
    {
        ...
        $row = $this->Tbl_pendaftaran_rekmed_model->get_by_id($id);
        if ($row) {
            $data = array(
        ...
        'alamat_penanggung_jawab' => $row->alamat_penanggung_jawab,
            ...
            $this->template->load('template','tindakanberobat/tbl_tindakan_read', $data);
        } else {
   ...
   function rujukan_action(){
    ...
    public function create() 
    {
        ...
        $data = array(
            'button' => 'Tambah',
            'action' => site_url('tindakanberobat/create_action'),
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        $pdf->Image('http://localhost/puskesmas/assets/foto_profil/logo-rs.png', 22, 5, 30);
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(268, 7, 'PUSKESMAS WADAS', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(268, 7, '          Jl. Pintu Air,Kp. Sindangsari, Desa Wadas No.17, Telukjambe Timur, Karawang.', 0, 1, 'C');
        $pdf->Cell(268, 7, 'Telepon : (0267) 4218872. E-mail: puskeswadas@gmail.com', 0, 1, 'C');
        $pdf->Line(10,35, 293-10, 35);
        $pdf->Line(10,35.5, 293-10, 35.5);
        $pdf->Cell(7, 7, '', 0, 1);
        $pdf->Cell(268, 7, 'Laporan Rekam Medis', 0, 1, 'C');
        ...
        $pdf->Cell(123, 7, ': '.$pasien['no_rekamedis'], 0, 0, 'L'); //90 merupakan jarak antara no_rekamedis dengan isian ditangani oleh

        $pdf->Cell(45, 7, 'Ditangani Oleh', 0, 0, 'L');
        $pdf->Cell(30, 7, ': '.$pasien['nama_dokter'], 0, 1, 'L');
        $pdf->Cell(30, 7, '', 0, 0, 'L');        
        $pdf->Cell(30, 7, 'Nama Pasien', 0, 0, 'L');
        $pdf->Cell(123, 7, ': '.$pasien['nama_pasien'], 0, 0, 'L');
        ...
        $pdf->Cell(69, 7, 'Hasil Periksa', 1, 0, 'C');
        ...
        $pdf->Cell(69, 7, $t->hasil_periksa, 1, 0, 'C');
        ...
        $pdf->Cell(130, 10, '', 0, 1);
        $pdf->Cell(30, 7, '', 0, 0, 'L');        
        $pdf->Cell(178, 10, 'Tanggal Cetak', 0, 0, 'R');
        $pdf->Cell(43, 10, ': '.date('d-m-Y '), 0, 0, 'R');
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
     ...
     function cetakrujukan(){
        ...
        $no_rawat = substr($this->uri->uri_string(3), 29);
      $sql_daftar = "SELECT pd.no_rekamedis, pd.no_rawat,pd.tanggal_daftar,ps.nama_pasien, td.nama_dokter FROM tbl_pendaftaran as pd, tbl_pasien as ps, tbl_dokter as td WHERE pd.no_rekamedis = ps.no_rekamedis and td.kode_dokter = pd.kode_dokter_penanggung_jawab and pd.no_rawat = '$no_rawat'";
      $sql_penyakit = "SELECT pd.no_rekamedis, pd.no_rawat,dg.nama_penyakit FROM tbl_pendaftaran as pd, tbl_diagnosa_penyakit as dg WHERE pd.no_rekamedis and pd.no_rawat = '$no_rawat'";
    ...
    public function create_action() 
    {
        ...
        'alamat_penanggung_jawab' => $this->input->post('alamat_penanggung_jawab',TRUE)
            ...
            $this->Tbl_pendaftaran_rekmed_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tindakanberobat/update_action'),
    ...
    public function update_action() 
    {
      ...
      $tanggal_daftar =  $this->input->post('tanggal_daftar',TRUE);
        ...
        'alamat_penanggung_jawab' => $this->input->post('alamat_penanggung_jawab',TRUE),
            ...
            $this->Tbl_pendaftaran_rekmed_model->update($this->input->post('no_rawat', TRUE), $data);
